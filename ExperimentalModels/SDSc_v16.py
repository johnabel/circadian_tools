...
y0in=np.array([0.23962902,  1.08818893,  0.18640473,  0.06997849,  0.00596862,
        0.67863339,  0.45778405,  0.4025576 ,  0.3883115 ,  0.04322924,
        0.36349864])
...
def main():
    ...
    sol = ODEsol.intODEs_sim(y0LC,300,numsteps=1000)
    tsol = ODEsol.ts
    periodsol = ODEsol.findPeriod(tsol,sol,'p')
    period = periodsol[0]
    
    trajectories = stk.stochkit(SSAmodel,job_id='coupled',t=500,number_of_trajectories=1,increment=0.1)
    StochPopEval = stk.StochPopEval(trajectories,state_names,param_names,vol,EqCount)
    StochPopEval.PlotPop('p',fignum=1)
    StochPopEval.PlotPop('eVIP',fignum=2)
    StochPopEval.PlotPop('c1',fignum=3)

    ODEplts = ctb.ODEplots(tsol,vol*sol,period,ODEmodel)
    ODEplts.fullPlot('p',figurenum=1)
    ODEplts.fullPlot('eVIP',figurenum=2)
    ODEplts.fullPlot('c1',figurenum=3)
