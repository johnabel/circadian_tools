...
class CircEval(object):
    ...
    def findPeriod(self,t,sol,StateVar):
        """
        This function will find the period of a solution of the system of ODEs. If there is no
        period, it will return a negative value. The inputs to this function are:
            
            t           -   the 1D array of time values
            sol         -   the solution array for the state variables. the period is calculated from
                            the first state variable, but the zero crossings of the other state
                            variables are also tested to ensure that the solution is periodic in all
                            state variables
            perCount    -   the number of periods to take before determining the standard deviation
                            default value is 8.
                            
        Sequence of operations: 
                                1. remove first 30% of t and sol values (cut transient)
                                2. subtract mean from sol, create index of 0s
                                3. find the times at which it crosses 0
                                4. determine each period length for first perCount periods
                                5. take standard deviation of periods, if less than pertol, accept.
                                6. check to ensure that other parameters are oscillating as well.
                                7. if too high stdev, or not enough 0s, return -1.
                                8. else, return period.
                                
                                **period as an output returns [period or error number, stdev]
        """
    ...
    def find_y0PLC(self, t, sol, period, StateVar):
        """
        ...
        #we now have the time of dawn. not the dawn of time.
        
    def ScaleParams(self,period,period_desired):
        """
        Draft method of scaling aprameters to fit desired period, DO NOT USE.
        """
        scalefactor = period/period_desired
        print self.param
        for i in range(self.ParamCount):
            if self.iv_pdict[i][0] is 'v':
                self.param[i]=self.param[i]*scalefactor
                print self.iv_pdict[i]
        print self.param
        return self.param
