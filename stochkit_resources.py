""" 
	...
	object. StochKitModel extends Model in the 'model.py' section, and
	supplements it with StochKit2 specific model serialization (to StochKit's
	naive XML format). 
...
class StochEval(object):
    ...
    def PlotAvg(self,SV,traces=True,fignum=1,color='black',conc=True):
        ...
        if traces is True:
            for i in range(self.trajcount):
                ...
                plt.plot(self.time,trj,color="gray",alpha=0.1)
        if conc==True:
            avg[:,statevarnum]=avg[:,statevarnum]/self.vol
        plt.plot(self.time,avg[:,statevarnum],color=color,label = SV)
    ...
    def PlotPop(self,SV,fignum=1,color='black',conc=False,traces=True,
        ...
        if traces is True:
            for i in range(len(SVindexes)):
                if conc==False:
                    plt.plot(self.time,self.trajectories[0][:,SVindexes[i]],
                             color='gray',#cmap(float(i) / len(SVindexes)),
                                alpha=0.1)
                else:
                    plt.plot(self.time,self.trajectories[0][:,SVindexes[i]]/self.vol,
                             color='gray',#color=cmap(float(i) / len(SVindexes)),
                                alpha=0.1)
...
#                             color='red')
        ...
        plt.plot(self.time,averageSV,color=color,label = SV)
    ...
    def RepIndivPlot(self,SV,Desc,period,tstart=0,tend=None,color='black',indiv =6):
        #Plot s representative individual neurons
        ...
        ax1.plot(plt_time, self.trajectories[0][start_ind:end_ind,
				SVindexes[0]], color=color)
        ax2.plot(plt_time, self.trajectories[0][start_ind:end_ind,
				SVindexes[1]], color=color)
        ax3.plot(plt_time, self.trajectories[0][start_ind:end_ind,
				SVindexes[2]], color=color)
        ax4.plot(plt_time, self.trajectories[0][start_ind:end_ind,
				SVindexes[3]], color=color)
        ax5.plot(plt_time, self.trajectories[0][start_ind:end_ind,
				SVindexes[4]], color=color)
        #ax6.plot(plt_time, self.trajectories[0][start_ind:end_ind,SVindexes[5]], color=color)
        
        ax1.set_ylabel('per mRNA')
        #ax4.set_ylabel('per mRNA')
        ax1.set_xlabel('Days')
        ax2.set_xlabel('Days')
        ax3.set_xlabel('Days')
        ax4.set_xlabel('Days')
